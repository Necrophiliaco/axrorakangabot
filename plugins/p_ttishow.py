    ...
    if temp.ME in r_j_check:
        ...
        if message.chat.id in temp.BANNED_CHATS:
            ...
            k = await message.reply(
                text="<b>🛑 CHATDA KERAKLI RUXSAT YO'Q 🐞\n\nAdminlar Ushbu Chatda Ishlashimni Taqiqlagan! Agar xatolik deb bilsangiz gruhda adminlarga yozin. . .</b>",
...
async def leave_a_chat(bot, message):
    ...
    try:
        ...
        await bot.send_message(
            chat_id=chat,
            text="<b>Salom Do'stlar, \nMening Adminlarim guruhdan ketishimni aytishdi shuning uchun xayr! Agar yana qo'shishni xohlasangiz admin bilan gaplashing!</b>",
            reply_markup=reply_markup,
        )
...
async def disable_chat(bot, message):
    ...
    try:
        ...
        await bot.send_message(
            chat_id=chat_, 
            text=f"<b>Salom Do'stlar, \nMening Adminlarim guruhdan ketishimni aytishdi shuning uchun xayr! Agar yana qo'shishni xohlasangiz admin bilan gaplashing!</b> \nSababi : <code>{reason}</code>",
            reply_markup=reply_markup)
...
async def ban_a_user(bot, message):
    ...
    if len(message.command) == 1:
        return await message.reply('Menga foydalanuvchi IDsi yoki Username ni bering!')
    r = message.text.split(None)
    if len(r) > 2:
        reason = message.text.split(None, 2)[2]
        chat = message.text.split(None, 2)[1]
    ...
    try:
        ...
        return await message.reply("Bunday Foydalanuvchi mavjud emas!")
    except IndexError:
        return await message.reply("Bu foydalanuvchi kanal orqali kirganga o'xshaydi. . .!")
    except Exception as e:
        return await message.reply(f'Error - {e}')
    else:
        jar = await db.get_ban_status(k.id)
        if jar['is_banned']:
            return await message.reply(f"{k.mention} is already banned\nSababi: {jar['ban_reason']}")
