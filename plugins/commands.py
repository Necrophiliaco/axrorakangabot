...
async def start(client, message):
    ...
    if AUTH_CHANNEL and not await is_subscribed(client, message):
        try:
            ...
            logger.error("Majburiy Obuna Kanaliga Bot Admin Ekanligini Tekshiring!")
            return
        btn = [
            [
                InlineKeyboardButton(
                    "❆ Kanalga qo'shilish ❆", url=invite_link.invite_link
                )
            ]
        ]
        ...
        if message.command[1] != "subscribe":
            try:
                ...
                btn.append([InlineKeyboardButton("↻ Tekshirish", callback_data=f"{pre}#{file_id}")])
    ...
    if data.split("-", 1)[0] == "BATCH":
        sts = await message.reply("<b>Iltimos Kuting. . .</b>")
...
async def delete(bot, message):
    ...
    if reply and reply.media:
        ...
        await message.reply("O'chirmoqchi bo'lgan faylga Reply qilib /delete deb yozing!", quote=True)
        return
    ...
    if result.deleted_count:
        await msg.edit("Muvaffaqiyatli O'chirildi")
    else:
        ...
        if result.deleted_count:
            ...
            if result.deleted_count:
                ...
                await msg.edit('Fayl Topilmadi')


@Client.on_message(filters.command('deleteall') & filters.user(ADMINS))
async def delete_all_index(bot, message):
    await message.reply_text(
        'Bu barcha fayllarni ochiradi.\nRostdan shuni xohlaysizmi??',
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        text="Xa", callback_data="autofilter_delete"
                    )
                ],
                [
                    InlineKeyboardButton(
                        text="Yo'q Jinni :)", callback_data="close_data"
                    )
                ],
            ]
        ),
    ...
    if chat_type == enums.ChatType.PRIVATE:
        ...
        if grpid is not None:
            ...
            try:
                ...
                await message.reply_text("Guruhda bor ekanimga ishonch xosil qiling!!", quote=True)
                return
        else:
            await message.reply_text("Bironta ham guruhga ulanmaganman!", quote=True)
            return
    ...
    if settings is not None:
        buttons = [
            [
                ...
                InlineKeyboardButton(
                    '1 dona' if settings["button"] else '2 talik',
                    callback_data=f'setgs#button#{settings["button"]}#{grp_id}',
                ),
            ],
            [
                ...
                InlineKeyboardButton(
                    'Qolda Boshlash' if settings["botpm"] else 'Aᴜᴛᴏ Boshlash',
                    callback_data=f'setgs#botpm#{settings["botpm"]}#{grp_id}',
                ),
            ],
            [
        ...
        btn = [[
                InlineKeyboardButton("Shu Yerda Ochish ↓", callback_data=f"opnsetgrp#{grp_id}"),
                InlineKeyboardButton("PMda Ochish ⇲", callback_data=f"opnsetpm#{grp_id}")
              ]]

        reply_markup = InlineKeyboardMarkup(buttons)
        if chat_type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
            await message.reply_text(
                text="<b>Sozlamalarni shu yerda ochishni xohlaysizmi ?</b>",
                reply_markup=InlineKeyboardMarkup(btn),
